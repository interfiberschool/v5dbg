...
Some literals such a `true` and `false` are in this documentation, their values **should not be interpreted as their String names** but instead their actual value. Hovering over literals will show their actual values values
...
The data field consists of a String value which corresponds to the completion status of the operat
