...
An example message can be seen as `%1:1:0`, message type one is program suspend. It should be noted that if more than three message separator characters are used then any located after the third are ignored and are merged into the message payload.
...
### `DEBUGGER_MESSAGE_RTHREADS` :octicons-list-ordered-24:{title="This message uses comma separated arguments in the data field"}
...
### `DEBUGGER_MESSAGE_RVSTACK` :octicons-checklist-24:{title="This message uses subarguments in the data field"}
...
### `DEBUGGER_MESSAGE_RLMEM` :octicons-checklist-24:{title="This message uses subarguments in the data field"}
